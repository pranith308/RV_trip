...
function rotRight(value: number, bits: number) {
...
function sha256Sync(bytes: Uint8Array) {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let i = 16; i < 64; i += 1) {
      const s0 = rotRight(w[i - 15], 7) ^ rotRight(w[i - 15], 18) ^ (w[i - 15] >>> 3)
      const s1 = rotRight(w[i - 2], 17) ^ rotRight(w[i - 2], 19) ^ (w[i - 2] >>> 10)
...
export async function sha256Hex(bytes: Uint8Array) {
  if (globalThis.isSecureContext && globalThis.crypto?.subtle) {
    try {
      ...
      /* Some mobile browsers expose subtle on HTTP but reject digest(). */
    }
  }
